    ...
    if kw_df.empty:
        ...
        if st.button("🚀 RSS 수집 시작", type="primary", disabled=not selected_kws):
            ...
            if not articles:
                ...
                # 발행일 신뢰도 요약
                reliable = inserted - rss_date_used - unknown_date
                with st.expander("📅 발행일 추출 결과 상세", expanded=(unknown_date > 0 or rss_date_used > 0)):
                    col_r, col_f, col_u = st.columns(3)
                    col_r.metric("원문 직접 추출 ✅", f"{max(reliable,0)}건")
                    col_f.metric("RSS 날짜 사용 ⚠️", f"{rss_date_used}건")
                    col_u.metric("날짜 미확인 ❌", f"{unknown_date}건")
                    if rss_date_used:
                        st.warning(
                            f"**{rss_date_used}건**은 원문에서 발행일을 찾지 못해 "
                            "Google News RSS 날짜를 저장했습니다. 실제 발행일과 다를 수 있습니다."
                        )
                    if unknown_date:
                        st.error(
                            f"**{unknown_date}건**은 발행일을 전혀 확인하지 못했습니다 (NULL 저장). "
                            "일간 브리핑 날짜 필터에서 제외될 수 있습니다."
                        )
